  ...
  return <div className="relative w-full rounded-2xl overflow-hidden bg-card border border-border shadow-lg mx-0 px-0">
      ...
      <div className="relative w-full h-0 pb-[35%] md:pb-[25%] bg-muted">
        ...
        <AnimatePresence mode="wait">
          ...
          duration: 0.5
        }} className="absolute inset-0 cursor-pointer" onClick={() => navigate(`/comunicados/${currentBanner.id}`)}>
            <img src={bannerImageUrl} alt={currentBanner.title} className={`absolute inset-0 w-full h-full object-cover ${getObjectPosition(currentBanner.imagePosition)}`} loading="eager" decoding="async" onLoad={() => setImageLoaded(true)} />
        ...
        {banners.length > 1 && <>
            {/* Navigation Arrows */}
            <Button variant="ghost" size="icon" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white backdrop-blur-sm border border-white/20 rounded-full h-10 w-10 transition-all" onClick={e => {
          e.stopPropagation();
          prevSlide();
        }}>
              <ChevronLeft className="h-5 w-5" />
            </Button>
            <Button variant="ghost" size="icon" className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/20 text-white backdrop-blur-sm border border-white/20 rounded-full h-10 w-10 transition-all" onClick={e => {
          e.stopPropagation();
          nextSlide();
        }}>
      ...
      {/* Bottom Indicators */}
      {banners.length > 1 && <div className="flex items-center justify-center gap-2 py-3 bg-muted/50">
          {banners.map((_, index) => <button key={index} onClick={() => {
        setImageLoaded(false);
