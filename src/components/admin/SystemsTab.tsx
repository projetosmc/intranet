...
export function SystemsTab() {
    ...
    if (oldIndex !== -1 && newIndex !== -1) {
      ...
      try {
        const updates = newOrder.map((item, index) => 
          supabase.from('tab_sistema').update({ num_ordem: index }).eq('cod_sistema', item.cod_sistema)
        );
  ...
  const SortableSystem = ({ system }: { system: System }) => {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: system.cod_sistema });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Status dos Sistemas</h2>
          <p className="text-sm text-muted-foreground">Gerencie os sistemas exibidos na página de status</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={(o) => { setIsDialogOpen(o); if (!o) { setEditingSystem(null); setFormErrors({}); } }}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="h-4 w-4 mr-2" />Novo Sistema</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSave(new FormData(e.currentTarget)); }} className="space-y-4">
              <div>
                ...
                <Input name="name" defaultValue={editingSystem?.des_nome} required maxLength={100} placeholder="Ex: Portal Cliente" className={formErrors.name ? 'border-destructive' : ''} />
              </div>
              <div>
                ...
                <Select name="status" defaultValue={editingSystem?.des_status || 'operational'}>
                  ...
                  <SelectContent>
                    <SelectItem value="operational"><div className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-success" /><span>Operacional</span></div></SelectItem>
                    <SelectItem value="degraded"><div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-warning" /><span>Degradado</span></div></SelectItem>
                    <SelectItem value="down"><div className="flex items-center gap-2"><XCircle className="h-4 w-4 text-destructive" /><span>Indisponível</span></div></SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label className="mb-1.5 block">Ordem</Label>
                <Input name="sort_order" type="number" defaultValue={editingSystem?.num_ordem || 0} placeholder="0" />
              </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <SortableContext items={systems.map(s => s.cod_sistema)} strategy={verticalListSortingStrategy}>
              <div className="divide-y divide-border">
                {systems.map((system) => (<SortableSystem key={system.cod_sistema} system={system} />))}
              </div>
            </SortableContext>
            <DragOverlay>{activeId ? <DragOverlayItem system={systems.find(s => s.cod_sistema === activeId)!} /> : null}</DragOverlay>
          </DndContext>
        )}
      </div>
      ...
      <AlertDialog open={!!deleteSystemId} onOpenChange={(open) => !open && setDeleteSystemId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Tem certeza que deseja excluir este sistema?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
